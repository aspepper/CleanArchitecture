...
namespace Acades.Saga.Utils
{
    internal static class TypeExtensions
    {
        ...
        static Dictionary<Type, Dictionary<Type, bool>> isThisTypeCache = new();

        internal static bool Is(this Type thisType, Type baseType)
        {
            lock (isThisTypeCache)
            {
                isThisTypeCache.TryGetValue(thisType, out Dictionary<Type, bool> baseTypeType);
                if (baseTypeType != null)
                {
                    if (baseTypeType.TryGetValue(baseType, out bool isResult))
                        return isResult;
                }
