    ...
    internal static class SagaActionExtensions
    {
...

        {
            ISagaAction action = actions.
                FindActionByStep(saga.ExecutionState.CurrentStep) ?? throw new SagaStepNotRegisteredException(saga.Data.ID, saga.ExecutionState.GetExecutionState(), saga.ExecutionState.CurrentStep);

            ISagaStep step = action.
                GetStep(saga.ExecutionState.CurrentStep) ?? throw new SagaStepNotRegisteredException(saga.Data.ID, saga.ExecutionState.GetExecutionState(), saga.ExecutionState.CurrentStep);
        ...
        {
            ISagaAction action = actions.
                FindActionByEventType(eventType) ?? throw new SagaInvalidEventForStateException(saga.Data.ID, saga.ExecutionState.GetExecutionState(), eventType);

            ISagaStep step = action.
                ChildSteps.GetFirstStep() ?? throw new SagaStepNotRegisteredException(saga.Data.ID, saga.ExecutionState.GetExecutionState(), saga.ExecutionState.CurrentStep);
